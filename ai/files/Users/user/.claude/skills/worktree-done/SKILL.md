...
- **EnterWorktree-managed (this session created it via the `EnterWorktree` tool):** do the merge manually so `ExitWorktree`'s session tracking stays in sync.
    1. Merge in the main checkout, all in one bash call: `cd <main> && git checkout <default> && git merge --ff-only <branch>`. Each bash tool call gets a fresh cwd (Claude Code does not persist cwd across calls), so step 2's main-checkout command must repeat the same `cd <main> &&` envelope rather than assume it inherits cwd from this step. If the FF fails, loop: run `git rebase <default>` inside the worktree (cwd is already there), then retry the FF in main. `<default>` can advance again between iterations (parallel work, hooks), so keep looping until the FF succeeds; bail if the rebase itself errors (e.g. conflict).
    2. Verify the commit is now reachable from the target branch: `cd <main> && git merge-base --is-ancestor <branch> <default>`. If this exits non-zero, the merge did not land; surface "ancestry check failed: <branch> is not reachable from <default> in <main>" and stop.
