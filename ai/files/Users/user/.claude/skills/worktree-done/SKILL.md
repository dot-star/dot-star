...
- **EnterWorktree-managed (this session created it via the `EnterWorktree` tool):** do the merge manually so `ExitWorktree`'s session tracking stays in sync.
    ...
    2. Fast-forward in the main checkout: `cd <main> && git checkout <default> && git merge --ff-only <branch>`. Each bash tool call gets a fresh cwd (Claude Code does not persist cwd across calls), so this command must include its own `cd <main> &&` envelope rather than assume it inherits cwd. If the FF still fails (e.g. `<default>` advanced again between the rebase and the merge), loop back to step 1; bail if the rebase errors. If the FF reports "Your local changes ... would be overwritten by merge" because `<main>` has unrelated uncommitted edits, stash them with `cd <main> && git stash push --message "worktree-done auto-stash before FF" -- <files>`, retry the FF, and `git stash pop` *after* step 4's `ExitWorktree`. Pop after the worktree is removed so a pop conflict resolves in a tidy state.
