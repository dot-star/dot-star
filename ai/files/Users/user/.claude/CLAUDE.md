...
- Self-documenting command variables and predicates: when a command's or test's intent isn't obvious from its literal text, bind it to a descriptively-named local variable or predicate function so the call site reads as prose.
