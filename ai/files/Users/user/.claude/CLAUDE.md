...
- For bracket-prefix choice asks (a question offering options for a one-token reply, not a full numbered list):
  ...
  - **Pre-send checklist**, run on every message before sending, not only on ones shaped as a question:
    ...
    2. Does a **statement** leave a decision open? A choice needn't be phrased as a question or carry an `or`: a nit weighing two variants ("Worth the two words."), a take-it-or-leave-it suggestion, or a recommendation attached to work already called final each hand the user a decision with nothing to reply with. Either bracket the alternatives or cut the observation. An invitation to reply ("say the word", "let me know", "happy to") is the tell: it asks for an answer the message never hands the user a token for.
       - **Repeat the nit's option in the how-to-proceed list, under the same letter.** An option offered only inline is invisible at the moment of choosing, since the reader scans the list and picks from what's there. Slot the row per the one-off rule (edit-and-defer sits under 🛠️ **`[i]terate`**) and word it as the concrete fix rather than restating the nit.
...
- When offering a worktree follow-up, present whichever of these bracket-prefix options apply to the moment (any subset the tree allows; ⬆️ promote and 🏁 land always apply), each on its own line led by its action emoji; never bundle two actions into one option (e.g. **`[p]romote and land`**). Whenever two or more appear together, list them top-to-bottom in this fixed order: iterate → commit → fold → promote → land. The slot order tracks least-to-most committal (don't-commit first, then commit-and-stay, then rewrite-and-stay, then promote, then the teardown); land is the only one that tears the worktree down, so it's always last, never floated into the middle or reordered.
  ...
  **Pre-send check**, run on every worktree menu:
    ...
    6. Does every decision the message body left open have a row here? Re-read the prose above for nits, 💅 polish notes, and 🔴 caveats; each one the user could accept needs its own row under the same letter, per the bracket-prefix checklist. A menu that omits one sends the reader back to typing a sentence.
