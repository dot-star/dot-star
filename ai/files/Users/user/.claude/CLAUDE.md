...
- For short inline prose binary/ternary asks (a single question in running text, not a numbered list), wrap each option's accept-prefix in `[...]` so the prompt is self-documenting. Example: "Want me to commit this as a [f]ollow-up, or [a]mend the previous commit?". Accept the bracketed prefix (case-insensitive) as a complete reply and map it back to the full option. Bracket every option; prefer the single-letter prefix (e.g. `[a]mend`), only extending to multi-character (e.g. `[am]end` vs `[ad]d`) when two options would otherwise share the same letter.
