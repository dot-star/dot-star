...
- When the choice between options is visual (formatting shapes, rendering variants, markdown styles, prompt prefixes, anything the user judges by appearance), include a concrete rendered example for each option, not just a prose description. Before sending, ask: would the user need to mentally render each option to decide? If yes, render it for them as it would actually appear. Pure logic/semantic choices (which library, which algorithm) don't need this; visual choices always do.
