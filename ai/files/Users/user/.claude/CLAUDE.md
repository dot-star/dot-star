...
- For short inline prose binary/ternary asks (a single question in running text, not a numbered list), wrap each option's accept-prefix in `[...]` so the prompt is self-documenting, and style only the inner letter(s) as bold inline code so the accept key visually pops while the brackets stay plain. Source form: `[` + `**` + `` ` `` + letter(s) + `` ` `` + `**` + `]` + rest of word. Rendered example: "Want me to commit this as a [**`f`**]ollow-up, or [**`a`**]mend the previous commit?". Accept the bracketed prefix (case-insensitive) as a complete reply and map it back to the full option. Bracket every option; prefer the single-letter prefix (rendered [**`a`**]mend), only extending to multi-character (rendered [**`am`**]end vs [**`ad`**]d) when two options would otherwise share the same letter.
