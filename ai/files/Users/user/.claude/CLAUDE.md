...
- Record an inferred fact as inferred: mark anything not stated plainly as unconfirmed and name what it was inferred from.
  - Read the source before claiming what a script, config, or tool does or doesn't do. A doc's paraphrase of it is a floor, never the whole set: `includes X` says nothing about whether `Y` is in there too, so a "won't match" or "doesn't support" claim built on a paraphrase is a guess dressed as a fact. One `grep` of the real list settles it; state the finding with the `file:line`.
  ...
  - Where a new piece of context goes:
    ...
    New context to add
     ...
     Needed in EVERY session?
       ├─ yes ─► ~/.claude/CLAUDE_<name>.md          (always-on hook)
       │
       └─ no ─► tied to a specific repo?
                │
                ├─ yes ─► ai/contexts/<name>.md
                │          + <repo>/CLAUDE.local.md:  @~/.dot-star/ai/contexts/<name>.md
                │          │
                │          └─ want a nudge when mentioned from elsewhere?
                │              ├─ yes ─► add  <!-- claude-mention: <keyword> -->
                │              └─ no  ─► done (loads only in that repo)
                │
                └─ mention-only ─► ai/contexts/<name>.md + claude-mention marker
    ```
...
- Name the step that makes a config change take effect whenever the edit lands in a file some long-running process already read: the reload keybind, the reload command, or a full restart. A correct file and live behavior are two different states; only the second is what the user asked for, so reporting the edit as done without the apply step reads to them as a fix that didn't work.
  - Verify before claiming it took effect: `ps -eo pid,lstart,comm` shows whether the process predates the edit. A new window, tab, split, or shell is not a reload, since a resident app hands those its in-memory copy.
...
- `x` always means "close this session": offer 🧨 **`[x]`** as a bracket-prefix option wherever closing is a sensible next step and spell out what closing entails at that spot. The bare letter is on purpose: `x` reads as "close this" from window chrome to vim and no other menu row claims it, so it never needs extending.
  ...
  - **`xx` on a worktree menu** is the close without the land: mark the session for prune, then `/exit`, touching nothing in git. No commit, no promote, no teardown; the worktree and its pending change stay on disk for a later session or `wts` to pick up. The pending change is what the row protects, so it renders only on a dirty tree: once every edit is committed the branch already holds the work, and the row would offer nothing but stranding that commit in an abandoned worktree. Typed on a clean tree anyway, `xx` still does what it says; it just isn't offered there. Never make it destructive: the prune mark already discards the conversation, so after it the worktree is the only record of the work; a stale worktree costs one `rm -r` where deleted edits are gone. A discard wants its own row with a destructive verb in its name, never a close key that happens to delete. Offer it as 🧨🧨 **`[xx]`** directly beneath 🧨 **`[x]`**; the doubled letter is `[x]`'s own sub-option the way `[xN]` is, so `[x]` keeps its bare letter and `x2` still picks draft 2. It carries no subject drafts, since nothing gets committed. Its recap is ⏸️, not 🏁, since the objective isn't complete: `⏸️ Session closed, worktree left as-is`, then the checklist rows `⏸️ Worktree kept (<name>); N files uncommitted` and `🧨 Session marked for prune`.
  - On `x` or `xx`, `Write` an empty file at `/tmp/claude/prune_marks/<session_id>` (the full id, as in the scratchpad path; `Edit(//tmp/claude/**)` is allowlisted so it needs no prompt, where anything under `~/.claude/` is a protected path that prompts whatever the allow rules say), then tell the user to `/exit`. `ai/claude/prune.sh` sweeps every marked session and its mark on its next run. The mark replaces `/rename d`: the built-in appends a collision suffix (`d2`) when another session already carries the title and that suffixed session never matches.
  - Delete the mark (`rm /tmp/claude/prune_marks/<session_id>`) when the user starts new work after picking `x` or `xx`, so the session survives.
...
- Two comment shapes:
  ...
  - **Line-doc comments** (describing the next line/pipeline): stay flush with the code they describe, no blank line between the comment and that code.
    - The "no blank line" applies only to the gap *below* the comment (comment-to-its-code).
    - A blank line *above* a line-doc comment is fine, and preferred, when it opens a new logical stanza, so the blank separates stanzas while the comment stays glued to its code below.
...
- Never restate a value in the comment documenting it. The comment carries the purpose and the tradeoff; the line below carries the value. Naming the current pick (`# Retry three times before giving up` above `max_retries = 3`) duplicates it in prose, so every retune drags a comment edit along and a skipped edit leaves the comment lying. Test it by imagining the other candidate values in that slot: if the comment has to change for any of them, it named the pick rather than the purpose.
  - Same for a heading, bold lead-in, or label over a list: name the set, never its size (`**Lines every option answers**`, not `**Four lines every option answers**`; `passes every one`, not `passes all four`). The list already carries the count, and a copy of it in the title goes stale on the first edit nobody thinks to mirror. Test it by imagining one more or one fewer item: if the heading would have to change, it named the count rather than the set.
- A comment carries what the code can't; anything recoverable from the lines below is noise. Cap a comment block at three lines of prose, about 30 words at standard wrap: past that it's explaining the domain rather than the line, so the explanation belongs in a doc or a commit body with the comment pointing there.
  - **Prose is what's capped.** A sample the reader scans rather than reads doesn't count: an example invocation, sample output, a column layout, a diagram. A 30-line before/after block costs a glance where four lines of explanation cost a paragraph's attention.
  - **The word figure is the backstop.** Three 150-character lines clear the line cap while carrying a paragraph, so a block busting 30 words is over however few lines it occupies.
  - **The test:** delete each sentence in turn, then ask whether a reader could recover it from the code below. Recoverable means cut, however well it reads.
  - **What survives:** a constraint invisible locally (why this call skips the flags its neighbor uses), the tradeoff behind a pick, the failure a guard prevents.
  - **What goes:** restating the mechanism the code already spells out, naming the platforms or commands a reader infers from the surrounding branch, the consequence of a consequence.
  - **Exempt:** a file-header or usage block, which documents the file rather than a statement. The cap governs a comment sitting over code.
- Lead every imperative line with a word that reads as a verb at a glance, not merely one that is a valid verb.
  - **Where it binds:** TODOs in code comments (`// TODO:`, `# TODO:`) and task-list entries (`todo.md`, `TODO`, `tasks.md`, README task sections), commit subjects and PR titles.
  - **Good leaders** are unambiguous: `Add`, `Show`, `Fix`, `Render`, `Plot`, `Wire`, `Move`, `Replace`, `Remove`, `Rename`, `Drop`.
  - **Bad leaders** read as nouns first: `Baseline`, `Brief`, `File`, `Mark`, `Plant`, `Surface`. They're valid verbs but parse as nouns at a glance, breaking scan-ability.
  - **The test:** read the first word alone, with no context. If it could open a noun phrase, pick another verb. `Baseline the new lint errors` reads as "the lint baseline"; `Silence` or `Ignore` reads as an instruction.
...
- Quote a sample phrase only when the exact characters matter; otherwise the quotes read as a magic phrase the reader has to type verbatim. When quoting one wording out of many that would work (a request a user might make, a command phrasing, a search term), name the category in the sentence and let the quoted text sit in a parenthetical as an example. Bad (reads as the trigger): `Say "let's pick up the next todo" and the rule fires.` Good: `Ask for the next todo in any wording (pick up the next one, what's next here) and the rule fires.` Quotes may stay inside that parenthetical when a marker labels the items as samples, whether `e.g.` in the parenthetical or a phrase like `in any wording` in the sentence; the marker is what defuses the verbatim reading, so `(e.g. "ship it", "land it")` needs no rewrite. A genuine exact-match trigger (a slash command, a literal token a script greps for) still gets quotes, since there the characters are the point.
- Generalize examples to neutral placeholders before they land in a committed file. When an illustration (code, comment, doc, test) would otherwise carry real private content (actual values from a private file, not invented samples), swap in invented placeholders showing the same pattern (`deploy`/`rollback`, `foo`/`bar`). Committed files are public, and an example only has to demonstrate shape, so its content is invented, never real data.
  - Count up through a well-formed identifier instead of copying a live one: a session id, UUID, hash or token becomes `12345678-90ab-cdef-1234-567890abcdef`, `1234567`, `abcdef123456`, still passing whatever format check the code applies. A random-looking value reads as real and may be (this session's own id is the common leak); a counting sequence reads as a placeholder at a glance.
...
- Interpret a bare one-token reply or a short fixed phrase as shorthand. Fires only when the entire message is exactly that token or phrase; if the message starts with a question word (what/how/why/is/should/can/does/...) it's a question *about* the token, not an invocation.
  ...
  **A bare number indexes rendered content, never an option's position.** Bracket options are picked by their letter alone, so a number never selects one no matter where it sits in the menu. It names one of the numbered items the previous message actually rendered: a finding, a list entry, a drafted variant. A number paired with a destination or a verb (`2 to cb`, `2 as a comment`) is always content, since a destination takes a thing rather than an action.
  ...
  - `s`, `st`, `sta` mean "give me a status, written for someone with no context": open with one line naming what we're trying to get done, then one bullet per item stacked vertically, each led by a status emoji as its only bullet marker. Cut detail before cutting the orientation; keep the whole thing scannable at a glance. Don't start new work. Close on where the work sits, one label per line. The markers:
    ...
    The labels below are the usual ones, not the whole set: omit any that don't apply and add anything else worth mentioning:
  ...
  - `tl`, `timeline` mean "render the events under discussion as a timeline table": one row per event, oldest first, each showing the gap from the row above. A channel style doc may specialize the layout.
...
- `r <x>` means "replace `<x>`": swap `<x>` for an alternate word or phrasing in whatever we're working on and leave the rest as it stands.
...
- Run the pre-send scans below before ending any message. A Stop hook block is expensive: the rejected draft stays on screen and the fix costs another turn.
  ...
  - Shape an acronym correction as a markdown table, one row per acronym the hook named, in that order, columns `Acronym | Meaning | In`, where In quotes a phrase carrying the acronym verbatim. The hook's list is already deduped, so an acronym the message used repeatedly still gets one row; a spelled-out occurrence is the fixed form rather than an offender, so it never earns one. Wrap the acronym in inline code in both the Acronym and In cells: the reader's eye lands on it and the hook strips code spans, so the table never re-trips it. The reader maps each row onto the message still on screen; a per-sentence rewrite makes them re-read prose they already have. Rendered example:
...
- Never emit `c`, `cb`, `r`, `trix`, or `wt` as standalone tokens in own output; always expand to "clipboard" / "root" / "spreadsheet" / "worktree". These are input-only shorthand (see the Shorthand section above) and reading them back as jargon obscures meaning.
  - A bracket-prefix option label is the exception, since the letters there are an accept token rather than prose: 📎 **`[cb]`** names the clipboard option (see the clipboard rule below). The ban still binds the surrounding sentence, so word the parenthetical "put it on the clipboard", never "to cb".
...
- Pick `a`/`an` by how the next token is said aloud, never by its spelling. A bare bracket token, lone letter, or initialism is said as letter names: `an [x] row` (said "ex"), `an SQL query`, `a URL`. A bracket letter with a remainder is said as the word: `a [f]old`, `an [a]dd row`. Binds commit subjects and code comments, not just chat prose.
  - Letter names opening on a vowel sound, so a bare one takes `an`: `a`, `e`, `f`, `h`, `i`, `l`, `m`, `n`, `o`, `r`, `s`, `x`.
  - Pre-send check: for every `a `/`an ` before a `[` or an all-caps token, say the token aloud.
- When citing a rule as broken, quote the exact characters that break it. A citation is a claim about specific characters in the text, not a general impression, so check the literal text against the rule before naming it. If you can't quote the offending substring, you don't have a violation: name what actually bothers you instead.
  - Fail (cites a rule the text doesn't break): "ends on a trailing period" of `Foo. Bar baz`, whose only period sits mid-message, which the rule allows
  - Pass (quotes the characters at issue): "the period after `Foo` reads flat"
...
- Make every option and example self-contained. When a numbered choice, a Fail/Pass line, or a rendered sample would lean on a sibling ("same as 2 but shorter", "same first line as above"), repeat the shared part instead. The reader compares entries by scanning down the list, so a cross-reference makes them hold one entry in their head while reading another, and a sibling that later gets cut or reordered strands the reference.
  - Fail (leans on option 1): `2. Same as 1, but with the retry capped`
...
- Open on a one-line description of any pasted image before acting on it, one bullet per image when several land together, each led by its `[Image #N]` label. The terminal renders a paste as `[Image #N]`, so the user can't see which image landed; naming what's in each one catches a wrong paste before work builds on it.
- For bracket-prefix choice asks (a question offering options for a one-token reply, not a full numbered list):
  ...
  - **Pre-send checklist**, run on every message before sending, not only on ones shaped as a question:
    ...
    2. Does a **statement** leave a decision open? A choice needn't be phrased as a question or carry an `or`: a nit weighing two variants ("Worth the two words."), a take-it-or-leave-it suggestion, or a recommendation attached to work already called final each hand the user a decision with nothing to reply with. Either bracket the alternatives or cut the observation. An invitation to reply ("say the word", "let me know", "happy to") is the tell: it asks for an answer the message never hands the user a token for.
    ...
    9. Does the message hand the user a command to run themselves? It needs a 📎 **`[cb]`** row, per the clipboard rule below.
- When a message hands the user a command to run themselves (a `! <command>` line, or a command paired with "run this"), offer 📎 **`[cb]`** to put it on the clipboard. Typing the command out by hand is the cost that offer removes, so the trigger is the user being the one who runs it: a command Claude already ran, or one quoted while explaining something, gets no row.
  - **Copy the form the user will paste:** keep the `!` prefix when the destination is this session's own prompt, drop it when they'll paste into a terminal.
  - **Wrap a script that may still change in `wf`:** hand it as `wf <script>` (`alias_watch_file` in `tools/bash/.aliases.sh`), so each save reruns it with no second paste; a script taking arguments uses `wf <script> "<command>"`. Copy it without `!`, since the watcher never exits and would hang the session's prompt.
  - **Reach for 📎, not 📋:** the loose-end 📋 **`[a]dd`** row already owns 📋 and both can share one menu.
  - **On a menu that also commits:** `[cb]` matches the `cb` clipboard shorthand and collides with commit on its first character, so that menu renders 💾 **`[co]mmit`** per the collision rule.
- When a message lays out two or more candidate versions of one tracked file (rewordings of an entry, alternative edits, a new sibling vs. an updated one), offer 👯 **`[v]ersions`** to build them as diffable mini repos. A diff beats a prose preview for judging an edit, so the offer hands the user the review they'd otherwise have to ask for.
  - **Recipe, per candidate N:** `git init version_N` in the session scratchpad, copy the file as committed in the real repo in as `versionN.<ext>` and commit it (the baseline), then overwrite it with candidate N and leave that uncommitted. `cd version_N && git diff` (or bare `d`) then shows exactly that candidate against the baseline. A candidate identical to the committed file gets no directory; say so instead.
  - **Mini repos, never worktrees or loose copies:** `git worktree add` registers on the real repo (`git worktree list`, sweep tooling, `wtd` semantics), where a mini repo is self-contained and cleanup is `rm -r`. A loose copy diffs as nothing, since one-arg `d <file>` routes to `rc_diff`, which only diffs a tracked path.
...
- When offering a worktree follow-up, present whichever of these bracket-prefix options apply to the moment (any subset the tree allows; ⬆️ promote, 🏁 land and 🧨 close always apply), each on its own line led by its action emoji; never bundle two actions into one option (e.g. **`[p]romote and land`**).
  ...
  - Land is the only git action that tears the worktree down, so it's always the last git row, never floated into the middle or reordered; 🧨 **`[x]`** and 🧨🧨 **`[xx]`** are the only rows allowed beneath it.
  - Close-as-is sits under close rather than up with iterate, its committal peer, because it's `[x]`'s sub-option and reads as "the other close" only next to its parent.
  ...
  **Applicability is a fact about the tree, not a guess.** Before sending, run `git status --porcelain` and branch on the result:
  ...
  - 🧨🧨 **`[xx]`**: mark the session for prune + `/exit`, touching nothing in git (no commit, no promote, no teardown), per the `xx` rule in Workflow. The worktree and its pending change stay on disk for a later session. Renders only on a dirty tree, since that pending change is what the row is for. Carries no drafts, since nothing gets committed.
  ...
  **A one-off option outside the standard rows** (e.g. 🧹 **`[cl]ean`** to delete dead code before committing) slots by how committal it is, next to the standard row it most resembles: edit-and-defer sits with 🛠️ **`[i]terate`**, edit-then-commit sits with the 💾 commit row. It never takes the top row from 🛠️ **`[i]terate`** or the bottom rows from 🏁 **`[L]and`**, 🧨 **`[x]`** and 🧨🧨 **`[xx]`**. It may fold its own commit in (as promote and land do), but never a second menu action's git state change. When its natural letter collides with a standard row's, both sides extend: a clean row turns the commit row into 💾 **`[co]mmit`**, so the menu reads 🧹 **`[cl]ean`** vs 💾 **`[co]mmit`**.
  ...
  > 　　 └─ **`[xN]`** ...
  >   🧨🧨 **`[xx]`**    (no commit + worktree left as-is + 🧨🧨 close session)
  ...
  Subset (clean tree, so no commit, fold or close-as-is slot; iterate reframed to keep tuning):
  ...
  The doubled emoji pushes the 🧨🧨 row's open-paren two cells right of the column; leave it, since widening every other row's padding costs more than the offset, which also marks the row as the odd one out.
  ...
  - Close-as-is is the close with the land taken out, so it sits beneath close as its variant.
...
- Whenever a message names loose ends (work this session surfaced but didn't do: a figure still unsourced, a file still to write, a decision the user has to make), offer 📋 **`[a]dd`** to bank them in a TODO section. Fires anywhere loose ends get named, not just at wrap-up: the 🏁 completion recap, a worktree follow-up, or a plain answer trailing off in "still needs". A loose end left in chat scrollback dies with the session; a TODO entry outlives it.
  ...
  **Slot order:** ⚡ **`[n]ow`** leads, then 📋 **`[a]dd`**, then the rest. Joining a worktree menu, both sit above 🛠️ **`[i]terate`** and the close rows still end the list.
...
　　 └─ `<older commit subject>`
    ...
    List only the steps that fired; for other objectives use whatever steps composed the work. A close-as-is (`xx`) leads with ⏸️ instead of 🏁, since nothing completed; its rows are spelled out in the `xx` rule in Workflow.
