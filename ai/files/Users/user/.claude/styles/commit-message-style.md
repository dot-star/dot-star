...
- Cut the precise value, token, or mechanism the diff already carries; the subject names *what* changed, not the exact figure/character/method it changed to. ``Fix checklist tree-row indent to one U+3000 full-width space`` → ``Fix checklist tree-row indent``; ``Bump retry timeout from 5s to 30s`` → ``Bump retry timeout``. The reader opens the diff for the how-much; keep only what stays meaningful without it.
