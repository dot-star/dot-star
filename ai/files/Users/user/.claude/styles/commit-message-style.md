...
- Lead a bundled subject with the heavier change and let the lesser parts follow. Docs, comments, and test updates riding along with a behavior change are the lighter half, so they read after it or drop out entirely: ``Fix the cache stampede and document the lock``, not ``Document the lock and fix the cache stampede``. A long subject gets trimmed from the end, so the ranking puts the cut where it costs least.
